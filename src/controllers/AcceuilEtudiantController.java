...
public class AcceuilEtudiantController implements Initializable {
    ...
    private List<Quiz> getAllQuiz_Test(String critere){
        List<Quiz> list  ;
        QuizDao qdao = QuizDao.getInstance() ;
        list = qdao.getAllQuiz(critere) ;       
       
       return list ;
    }
    
    private List<Test> getAllTest(String critere){     
        List<Test> listTest ;
        TestDao tdao = TestDao.getInstance() ;
        listTest = tdao.getAllTest(critere) ;
        return listTest; 
    }
